...
for (const dir of dirs) {
  ...
  if (fs.existsSync(entriesFile)) {
    try {
      ...
      if (content.includes(targetProject) && content.includes('src')) {
        const data = JSON.parse(content);
        const res = decodeURIComponent(data.resource);
        
        // Extract the filename regardless of path format
        const match = res.match(/jewellery-bill-book[\/\\](.*)$/i);
        if (match) {
            const relPath = match[1].replace(/\\/g, '/'); // normalize to forward slashes
...
console.log("Total files restored this run:", restored);
